...
// ==================== FUZZY MAMDANI (NODE EXHAUST - TIDAK BERUBAH) ====================
struct FuzzyResult {
  float mu_temp_low;
...
// Fungsi ini dipanggil saat gateway menerima data dari node nutrient (node 3/4)
// Data fuzzy lengkap dikirim dari node via field "fuzzy_detail"
bool sendFuzzyNutrientToFirebase(int node_id, const DynamicJsonDocument& doc) {
  ...
  // Inferensi (firing strength 9 rules)
  ...
  if (success) {
    Serial.printf("[FUZZY-FB] ✅ Node %d fuzzy log sent to /fuzzy_log/node_%d\n", node_id, node_id);
  } else {
...
void streamCallback(FirebaseStream data) {
  ...
  if (path.startsWith("/nodes/node_")) {
    ...
    int nodeId = path.substring(nodeStart, nodeEnd).toInt();
...
void handleNodeSync() {
  ...
  // ==================== FUZZY: EXHAUST NODE (tidak berubah) ====================
  ...
  // ==================== FUZZY LOG: NUTRIENT NODE SAJA (node 3/4) ====================
  // Hanya dikirim jika data berasal dari node nutrient (ada field "ppm" dan "fuzzy_detail")
...
void handleResetPDR() {
  ...
  if (node_id == 0) {
    for (int i = 0; i < NUM_NODES; i++) {
      pdr_stats[i].packets_sent = 0; pdr_stats[i].packets_received = 0;
      pdr_stats[i].packets_lost = 0; pdr_stats[i].pdr_percentage = 0.0f;
      pdr_stats[i].status = "No Data"; pdr_stats[i].last_reset = millis();
    }
    Serial.println("[PDR] ✓ Reset all nodes");
  } else if (node_id >= 1 && node_id <= NUM_NODES) {
    int idx = node_id - 1;
    pdr_stats[idx].packets_sent = 0; pdr_stats[idx].packets_received = 0;
    pdr_stats[idx].packets_lost = 0; pdr_stats[idx].pdr_percentage = 0.0f;
    pdr_stats[idx].status = "No Data"; pdr_stats[idx].last_reset = millis();
